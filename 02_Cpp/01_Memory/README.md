...
> 날짜: 2026-04-11 | 수업 7 마무리

## 수업 7 전체 핵심 정리

### 7-1. 스택 vs 힙
```
스택: 자동 관리, 빠름, 1~8MB, 함수 끝나면 해제
힙:   수동 관리, 느림, 큼, 직접 해제할 때까지 유지
게임 오브젝트는 힙 → 함수 끝나도 살아있어야 함
```

### 7-2. new/delete 누수 5패턴
```
1. 단순 누락  2. 조기 return  3. 예외
4. 포인터 덮어쓰기  5. 컨테이너 clear()

new ↔ delete, new[] ↔ delete[] (짝 필수)
```

### 7-3. unique_ptr
```
단독 소유, 복사 불가(이중 해제 방지), std::move로만 이전
성능 오버헤드 0 → 기본 선택
```

### 7-4. shared_ptr / weak_ptr
```
shared_ptr: 참조 카운트, 0이면 자동 delete
weak_ptr:   카운트 안 올림 → 순환 참조 해결, lock()으로 사용
```

### 7-5. 실습 교훈
```
- weak_ptr은 shared_ptr에서만 만들 수 있다
- 멤버 변수는 클래스 안에서 한 번만 선언
- 템플릿 <> 안에는 타입, 변수명 아님
```
...
| Q2 | 배열 해제 (`delete[]`) | **X** |
| Q3 | 조기 return 누수 | O |
| Q4 | unique_ptr 복사 불가 이유 | O |
| Q5 | unique_ptr 이동 함수 (`std::move`) | **X** |
| Q6 | shared_ptr 참조 카운트 | O |
| Q7 | 순환 참조 문제 | O |
| Q8 | weak_ptr 접근 (`lock()`) | O |
| Q9 | shared_ptr 소멸 시점 | O |
| Q10 | weak_ptr로 저장하는 이유 | O |

### 약점 정리

1. **`delete[]`** — 배열 해제 시 대괄호 필수. `new[]` 짝꿍.
2. **`std::move`** — unique_ptr 이동 시 필수. 함수 이름 혼동 주의.

### 강점

- 메모리 구조, 누수 패턴, 순환 참조 개념 확실
- weak_ptr / lock() 이해 완벽
- shared_ptr 참조 카운트 메커니즘 이해

---

> **수업 7 (메모리 관리) 완료!** 다음: 수업 8 다형성 심화 (4/13~)
